...
| 3 | Pre-commit defect prevention | pre_commit_review, pre_push_audit | [`05-pre-commit-gates.md`](05-pre-commit-gates.md) | **slices 1-5 IMPLEMENTED + deployed 2026-08-29** (777c951 gate outcomes; 9da8522 DEGRADED; 6f149c8 in-gate caps live; af737da unwired failed-lookup ⇒ skip; A6 pre_push_audit tally + INACTIVE on an empty rule namespace in release14). Remaining: OciusX rule ingestion is a user action (ADO PAT) |
| 4 | Exact entity/consumer discovery | get_concept_footprint, find_symbol_references | [`04-concept-and-consumer-discovery.md`](04-concept-and-consumer-discovery.md) | **slices 1-7 IMPLEMENTED + deployed 2026-08-29** (… 2b85769 find_symbol_references caps; A9 every incoming count names its rule in release14). Open: A4 aliases, A6 consumer classification |
...
| 8 | Security / settings / durable laws | map_guards_and_settings, repo rules, immune_check | [`08-guards-and-settings.md`](08-guards-and-settings.md) | **slices 1-4 IMPLEMENTED + deployed 2026-08-29** (95ac7d8, 8ff187f, 195d3c8, f4bc30f branch-only own check ⇒ helper credited). Live after release13: the credited helper was `GetByID`/check_pr_id — the object scoping lives inside the DAL and bare in-class helpers have no Calls edge ⇒ slice 5 (in-file helper candidates, `level: object` from the family). Open: B policy model |
