...
| 3 | Pre-commit defect prevention | pre_commit_review, pre_push_audit | [`05-pre-commit-gates.md`](05-pre-commit-gates.md) | **slice 1 IMPLEMENTED + deployed + live 2026-08-28** (777c951; per-gate outcomes, never green on a failed/panicked gate; §7 live 17/17 gate_status). Next: A3 per-gate degraded |
| 4 | Exact entity/consumer discovery | get_concept_footprint, find_symbol_references | [`04-concept-and-consumer-discovery.md`](04-concept-and-consumer-discovery.md) | **slices 1-3 IMPLEMENTED + deployed + live 2026-08-28** (bc15026, 09aac51, 1499b3f; G1 25 % → 35 % → **94 %** after reindex). Slice 4 in flight: case-preserving trigram index misses case variants (a `grep_project` quiet failure, §7b) |
...
| 7 | Causal UI/data tracing | trace_ui_event, trace_data_flow, find_connection_path | [`06-causal-trace-engine.md`](06-causal-trace-engine.md) | **slice 1 green locally 2026-08-28** (method-node attribution, every call a step, markdown+JSON, hop parity; 4 RED→GREEN) — sweep/deploy/live pending; slice 2 = bounded follow (A3) |
