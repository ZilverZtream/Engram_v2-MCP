...
| Item 8 ImpactEngine — causal golden suite first, then the one-hop slice (TS→API route resolution) | IN PROGRESS — fixed@5aad54a (route resolution: js name-route edges, VB dispatch_key, GraphStore::resolve_route_edges) + 2bcb084 (arm scan on the Roslyn sidecar path, ingest same-key metadata union, class-scoped dispatch + arm precedence, planner causal Usage cues + technology-noun stoplist + typed lowercase mentions, named-file callee hop seeded first); live r43 (2026-08-30 19:06) then r44 (20:29) | Causal golden suite eval/data/ask_causal_ociusx.jsonl (20 questions: who calls X / what breaks if Y / what does Z call / TS→API routes; ground truth generated read-only from the broker's `Case` arms and the TS call sites; git-ignored, sha256 in every --out record): r42 baseline 5/20 (evidence/causal_golden_r42_baseline.*), r43 10/20, r44 19/20 (evidence/causal_golden_r44.*). Live graph r44: route pass `api_calls=432 added=313 ambiguous=1 unbound=1`; api.getimg 0→1 TS caller, DeleteChangeRequest and ioCheckCanUserDeleteOrMoveMarker (renamed implementations) bound through the broker arm, iopGetAvailableImages/fjGet/fmUpdateStatus/prGetSubProjects list their TS clients (evidence/verify_r44.log). Golden 35: r44 33/35 — ox_multi_4 (2-hop wrapper route + 'marker info window' compound entity, unchanged since r42) and ox_history_2 (REGRESSION from 2bcb084: the typed-mention rule took the verb 'update' in 'update endpoint' as a symbol, so a correct abstention became an answer without PR evidence). Ref-story ranks 6/6 twice (co-change complete; 5/6 right after a wipe is the warm-up transient). OPEN → cycle 28: resolver binds an API-name literal to its implementation through the arm (ox_causal_1), verb stoplist for typed mentions (ox_history_2), 'merged PR' history cue. |
