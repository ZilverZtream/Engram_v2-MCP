...
| P0-4 answer-correctness golden suite | fixed@6ad6932 + 001a6ab + 96185a2 + bfcaa38; live r41 33/35 (2026-08-30 16:45) — P0-4f modality slots queued; multi_4 = route resolution (item 8) | Golden harness judges EVIDENCE per question (required_modality / required_all / forbidden / min_precision; 31/35 rows carry requirements; corpus git-ignored by the no-customer-strings rule, referenced by sha256). Honest baseline r33 27/35 → r37 32/35 → r39 29/35 (callee flooding) → r40 31/35 → r41 33/35, status-match 35/35 throughout. Engine: Modality on the plan, suffix filter inside the index on both legs, a filtered arm per requested modality, one bounded call-graph hop (intent-gated, name-cued, one callee per file, max 3), ONE reserve pass (needed kind / requested modality preferring the question's words / named file) with protected eviction, at most two items per file, hyphenated file names as entities. LIVE r41: the auditor's three questions cite .resx / .rdl ×5 / .sql; multi_1 answered via CanUserBulkUpdate one hop from the API entry point. Remaining: exact_4 precision 0.40 (one reserved .sql among ten items — P0-4f reserves up to three modality items), multi_4 api-images (the TS→API edge does not exist in the graph: route resolution — ImpactEngine, item 8) |
