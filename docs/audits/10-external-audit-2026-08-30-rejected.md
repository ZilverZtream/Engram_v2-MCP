...
| P0-2 completeness = path-set integrity per store | fixed@54cfa55 + live r35 (2026-08-30 10:24); follow-up queued (cycle 22: lossy UTF-8 decode, per-generation skip ledger, tolerance 0) | Path sets per store replace the chunk/file ratio: LIVE health `generation 837 — expected paths: 2277; tantivy: 2272, vectors: 2272, graph: 2277; missing: 5, extra: 0, cross-store mismatch: 0; chunks 31225 / vector rows 31225 (diagnostic)`, missing sample listed, no percentage; freshness `generation_complete: true (2272 of 2277 eligible paths present in every store; missing 5, cross-store mismatch 0, tolerance 22)`; check_integrity 172,107 = 172,107. The 5 missing are REAL: 4 Latin-1 stylesheets/scripts + 1 vendor min.js skipped at ingest as 'Invalid UTF-8 encoding' (fingerprinted → graph File node, no Tantivy/LanceDB docs; `grep_project` finds none of their content). Tests: index_integrity_pathset_tests (path sets, masking defect, one-sided vector loss, sample paths). Tolerance (max(3, 1 %)) hid this — follow-up makes decode lossy, persists skip reasons per generation and sets tolerance 0 |
| P0-3 change-set precision/ranking | fixed@06219bd + a936cf2 (r36/r37); live 5/6 — P0-3c layer-head promotion queued (cycle 22) | LIVE r37 reference story (4 s): 174 candidates, PRIMARY 40 (cap), 43 omissions; broad terms refused and named ('main' 860 files, 'reporting' 358, 'category' 286, 'kategori' 117); ranks: page 8, code-behind 9 (`name`+`vector`, were 121/122 → 42/43 → 8/9), api-redovisning.vb 15, redovisningskategorier.vb 17, iFalt.dbml 26, rk_redovisningskategorier.sql 37 (3 signals behind 4-signal rows) → 5/6 within top 30. Tests: change_set_rank_tests (rank acceptance, broad noise never primary, name coverage, markdown order) + unit story_word_name_coverage_leads_its_tier. Remaining: the best row of each LAYER belongs in the primary head (the only Data-layer critical file is the SQL) — cycle 22 |
