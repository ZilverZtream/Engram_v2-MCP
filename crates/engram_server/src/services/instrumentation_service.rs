...
use engram_core::runtime_evidence::{
    RuntimeEvidenceBatch, RuntimeEvent, RuntimeEventType,
};
...
pub fn generate_instrumentation_code(
    ...
    if has_postback {
        captured_events
            .push("Control interactions (postback source, event argument)".into());
    }
    ...
    let installation_steps = vec![
        ...
        "4. Deploy and exercise the application normally".into(),
        format!(
            "5. Collect the generated JSON logs and feed them to the {} tool",
...
fn generate_csharp_module(has_session: bool, has_sql: bool, has_postback: bool) -> String {
    ...
    let _ = writeln!(code, "// Auto-generated by Engram-MCP — Runtime Instrumentation Module");
    ...
    if has_postback {
        let _ = writeln!(code, "        app.PostMapRequestHandler += OnPostMapRequestHandler;");
    }
    let _ = writeln!(code, "    }}");
    let _ = writeln!(code);
    ...
    // BeginRequest
    let _ = writeln!(code, "    private void OnBeginRequest(object sender, EventArgs e)");
    let _ = writeln!(code, "    {{");
    let _ = writeln!(code, "        var ctx = HttpContext.Current;");
    let _ = writeln!(code, "        ctx.Items[\"engram_start\"] = Stopwatch.StartNew();");
    ...
    let _ = writeln!(code, "            timestamp = DateTime.UtcNow.ToString(\"o\")");
    let _ = writeln!(code, "        }});");
    let _ = writeln!(code, "    }}");
    let _ = writeln!(code);

    // EndRequest
    let _ = writeln!(code, "    private void OnEndRequest(object sender, EventArgs e)");
    let _ = writeln!(code, "    {{");
    let _ = writeln!(code, "        var ctx = HttpContext.Current;");
    let _ = writeln!(code, "        var sw = ctx.Items[\"engram_start\"] as Stopwatch;");
    let _ = writeln!(code, "        LogEvent(new {{");
    let _ = writeln!(code, "            event_type = \"route_complete\",");
    let _ = writeln!(code, "            source_path = ctx.Request.Path,");
    let _ = writeln!(code, "            target = ctx.Response.StatusCode.ToString(),");
    let _ = writeln!(code, "            duration_ms = sw?.ElapsedMilliseconds ?? -1,");
    let _ = writeln!(code, "            timestamp = DateTime.UtcNow.ToString(\"o\")");
    let _ = writeln!(code, "        }});");
    let _ = writeln!(code, "    }}");
    let _ = writeln!(code);
    ...
    let _ = writeln!(code, "            source_path = ctx.Request.Path,");
    let _ = writeln!(code, "            target = ex?.GetType().Name ?? \"unknown\",");
...
fn generate_vb_module(has_session: bool, has_sql: bool, has_postback: bool) -> String {
    ...
    let _ = writeln!(
        code,
        "Public Class EngramInstrumentation"
    );
...
mod tests {
    ...
    fn index_runtime_events_groups_by_path() {
        let events = vec![
            test_event(RuntimeEventType::Route, "/Orders.aspx", Some("GET"), "2024-01-01T00:00:00Z"),
            test_event(RuntimeEventType::Route, "/Orders.aspx", Some("POST"), "2024-01-01T00:01:00Z"),
        ];
    ...
    fn check_runtime_hit_finds_sql() {
        let events = vec![
            test_event(RuntimeEventType::SqlExecution, "fn:LoadOrders", Some("SELECT * FROM Orders"), "2024-01-01T00:00:00Z"),
        ];
        let idx = index_runtime_events(&events);
    ...
    fn check_runtime_hit_finds_state() {
        let events = vec![
            test_event(RuntimeEventType::StateMutation, "/Page.aspx", Some("Session:UserId"), "2024-01-01T00:00:00Z"),
        ];
        let idx = index_runtime_events(&events);

        let hit = check_runtime_hit(
            "fn:Page_Load",
            "Session:UserId",
            "reads_state",
            &idx,
