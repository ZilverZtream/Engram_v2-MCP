...
static RE_IS_POSTBACK_CS: LazyLock<Regex> =
    LazyLock::new(|| Regex::new(r"(?i)if\s*\(\s*(!?\s*(?:Page\.)?IsPostBack)\s*\)").unwrap());

static RE_IS_POSTBACK_VB: LazyLock<Regex> =
    LazyLock::new(|| Regex::new(r"(?i)If\s+(Not\s+)?(?:Page\.)?IsPostBack\b").unwrap());
...
pub fn analyze_page_lifecycle(
    ...
    for cap in lifecycle_re.captures_iter(codebehind_content) {
        ...
        let line_num = find_line_number(codebehind_content, cap.get(0).unwrap().start());

        let body = extract_method_body(codebehind_content, cap.get(0).unwrap().start(), is_vb);
